!#  File name : imagsph-th.f90	!# Last modified : 26 April 2017
...
! normalizes the wave function, CHEM() and RADS() are used to calculate the 
...
!------------------------ INTERFACE BLOCKS -----------------------
  INTEGER :: K,   NO_OF_THREADS
   REAL (8):: ZNORM, MU, RMS, EN, T1, T2, GSTP 
...
! INITIALIZE() initializes the starting normalized wave function 'CP'.
  CALL INITIALIZE()
!
  OPEN(7, FILE = 'imagsph-th-out.txt')
...
!   CP2=CP*CP
!   OPEN(1, FILE = 'imagsph-th-den-init.txt')
...
!    CP2=CP*CP
!    OPEN(2, FILE = 'imagsph-th-den-nstp.txt')
...
!
  ...
  IF(NPAS /= 0)THEN    
    ...
    1008 FORMAT('After NPAS iter.:',F8.4, 2F12.5, 2F11.5)
!    CP2=CP*CP
!    OPEN(3, FILE = 'imagsph-th-den-npas.txt')
...
!
     CP2=CP*CP
    OPEN(4, FILE = 'imagsph-th-den.txt')
...
!----------------------------------------------------------------------
  ...
  REAL (8), DIMENSION(0:SIZE(CP)-1) :: DCP, TMP1D, EMP1D
!
  DCP = DIFF(CP, DX)
  DP2 = DCP(1:)*DCP(1:)
  P2 = CP(1:)*CP(1:)
...
!
 DO I = 0, N
    IF (I == 0) THEN
        WRITE(FUNIT, 999) X(I), U2(1)/X(1)**2
    ELSE
        WRITE(FUNIT, 999) X(I), U2(I)/X(I)**2
    END IF
  END DO
!  DO I = 0, N
