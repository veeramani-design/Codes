...
MODULE COMM_DATA
    ! NX, NY, NZ : Number of space mesh points (X, Y and Z)
      INTEGER, PARAMETER :: NX = 128, NXX = NX-1, NX2 = NX/2 ! No. of X steps NX = 256
      INTEGER, PARAMETER :: NY = 128, NYY = NY-1, NY2 = NY/2 ! No. of Y steps NY = 256
      INTEGER, PARAMETER :: NZ = 16, NZZ = NZ-1, NZ2 = NZ/2 ! No. of Z steps NZ = 32
    ...
    MODULE GPE_DATA
      ...
      REAL (8), PARAMETER :: AHO = 1.0D-6 ! Unit of length (l= 1 MICRON) SCALING FACTOR
      REAL (8), PARAMETER :: LHO = 0
      REAL (8), PARAMETER :: GHO = 0.004D0 ! G_3D=0.1
      REAL (8), PARAMETER :: Bohr_a0 =  5.2917720859D-11/AHO ! Bohr radius (scaled with AHO)
      COMPLEX (8), PARAMETER :: CI = (0.0D0,1.0D0)
    
      REAL (8), PARAMETER :: DX = 0.05D0, DY = 0.05D0,  DZ = 0.025D0 ! DX, DY, DZ : Space step DX = 0.05D0, DY = 0.05D0,  DZ = 0.025D0 and DT : Time step 
    ...
    ! INTEGER, PARAMETER :: RANDOM=0 ! No random phase in the initial function
      ...
      REAL (8), PARAMETER :: DT = 0.00025D0    !  Imaginary time propagation, Time step DT = 0.00025d0 DT = 0.016D0
    ...
    ! G_3D : Nonlinearity in the 3D GP equation
      REAL (8), PARAMETER :: G_3D =  4.D0 * PI * AS * NATOMS * GHO
      REAL (8), PARAMETER :: G_2D = G_3D * SQRT(LAMBDA/(2*pi))
      REAL (8), PARAMETER :: LHY = LHO* 128*PI**0.05D0*AS**2.5D0*NATOMS**2.5D0/3   ! LHY correction term 
      REAL (8), PARAMETER :: OMEGA = 0.0D0 ! Angular Frequency
    ...
    SUBROUTINE CALCNU(CP, DT) ! Exact solution
      ...
      USE GPE_DATA, ONLY : OPTION, V, G, OPTION_RE_IM, CI, LHY
      IMPLICIT NONE
      COMPLEX (8), DIMENSION(0:NX,0:NY,0:NZ), INTENT(INOUT) :: CP
      REAL (8), INTENT(IN) :: DT
    
      REAL (8), DIMENSION(0:NX,0:NY,0:NZ) :: P2,  TMP3D, P3
      COMPLEX (8) :: CIJ
      INTEGER :: I, J, K
      ...
      DO K = 0, NZ; DO J = 0, NY; DO I = 0, NX
        ...
        P3(I,J,K)  = CP(I,J,K) * CONJG(CP(I,J,K)) * CP(I,J,K)
        TMP3D(I,J,K) = (V(I,J,K) + G * P2(I,J,K) + LHY * P3(I,J,K))
    ...
    SUBROUTINE CHEM(CP, MU, EN)
      ...
      USE COMM_DATA, ONLY : NX, NY, NZ
      USE GPE_DATA, ONLY : DX, DY, DZ, V, G, OMEGA, X, Y, CI,XOP, LHY
      IMPLICIT NONE
      COMPLEX (8), DIMENSION(0:NX,0:NY,0:NZ), INTENT(IN) :: CP
      REAL (8), INTENT(OUT) :: MU, EN
      ...
      INTEGER :: I, J, K
      REAL (8), DIMENSION(0:NX, 0:NY, 0:NZ) :: P2, GP2, DP2, CPR,CPI,P2R, P3
      REAL (8), DIMENSION(0:NX, 0:NY, 0:NZ) :: TMP3D, EMP3D, DPLZ, LHY2
      ...
      GP2 = G*P2
      LHY2 = LHY*P3
      TMP3D = (V + GP2 + LHY2/2.0D0) * P2R + DP2 - XOP * (OMEGA * DPLZ)
      EMP3D = (V + 0.5D0 * GP2 + LHY2*2.0D0/5.0D0) * P2R + DP2 - XOP * (OMEGA * DPLZ)
