!#  File name : realcir-th.f90	!# Last modified : 26 April 2017
...
MODULE GPE_DATA
  ...
  INTEGER, PARAMETER  :: NATOMS = 400
  REAL (8), PARAMETER :: AS = 118.2516754D0*Bohr_a0
  ...
  REAL (8), DIMENSION(:), ALLOCATABLE :: X, X2, X3, V
...
MODULE CN_DATA
  ...
  COMPLEX (8), DIMENSION(:), ALLOCATABLE :: CTT, CAL, CGA, CBE
...
! COEF() defines the coefficients of Crank-Nicholson Scheme.
  CALL COEF()
!
  OPEN(7, FILE = 'realcir-th-out.txt')
...
!
  ...
  901 FORMAT('  Number of Atoms N =',I9,', Unit of length AHO =',F12.8,' m')
  902 FORMAT('  Scattering length a = ',F9.2,'*a0')
  910 FORMAT('  Nonlinearity G_3D =',F16.7)
  903 FORMAT('  Nonlinearity G_CIR =',F15.7)
  909 FORMAT('  Axial trap parameter =',F8.3)
  905 FORMAT(' # Space Stp: NX = ', I8)
  906 FORMAT('  Space Step: DX = ', F12.6)
...
!
!   OPEN(1, FILE = 'realcir-th-den-init.txt')
...
!
!    OPEN(2, FILE = 'realcir-th-den-nstp.txt')
...
!
  T = 0.D0  
  IF((NPAS /= 0).OR.(NRUN /= 0)) OPEN(8, FILE = 'realcir-th-dyna.txt')
...
!
     OPEN(3, FILE = 'realcir-th-den.txt')
...
!
!     OPEN(4, FILE = 'realcir-th-nrun-den.txt')
...
!
  ...
  DO J = 1, NX
     CTT(J) = CDT/(4.D0*DX*X(J))
     CAIMX(J) = CAIPMX - CTT(J)
     CAIM = CAIPMX + CTT(J)
     CAL(J+1) = CGA(J)*CAIM
     CGA(J+1) = -1.D0/(CAI0+(CAIPMX-CDT/(4.D0*DX*X(J+1)))*CAL(J+1))  
  END DO
END SUBROUTINE COEF
