!#  File name : realsph-th.f90	!# Last modified : 26 April 2017
...
MODULE GPE_DATA
  ...
  REAL (8), PARAMETER :: AS = 94.351186D0*Bohr_a0
...
! time propagation of derivative part. NORMS() calculates the norm and 
...
!
  INTERFACE
    SUBROUTINE WRITE_DENSITY(FUNIT, U)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: FUNIT
      COMPLEX (8), DIMENSION(0:), INTENT(IN) :: U
    END SUBROUTINE WRITE_DENSITY
  END INTERFACE
!------------------------ INTERFACE BLOCKS -----------------------
  INTEGER :: K
  REAL (8) :: ZNORM, MU, RMS, EN, T, T1, T2, GSTP 
...
! COEF() defines the coefficients of Crank-Nicholson Scheme.
  CALL COEF()
!
  OPEN(7, FILE = 'realsph-th-out.txt')
...
!
!   OPEN(1, FILE = 'realsph-th-den-init.txt')
!    CALL WRITE_DENSITY(1, CP)
!   CLOSE(1)
!
! NORMS() calculates and restore normalization.
! CHEM() calculates the chemical potential MU and energy EN.
! RADS() calculates the r.m.s radius RMS.
...
!
!    OPEN(2, FILE = 'realsph-th-den-nstp.txt')
!     CALL WRITE_DENSITY(2, CP)
!    CLOSE(2)
!
  ELSE
    G = XOP * G_3D
...
!
  T = 0.D0  
  IF((NPAS /= 0).OR.(NRUN /= 0)) OPEN(8, FILE = 'realsph-th-dyna.txt')
  911 FORMAT(F12.6, F16.8)
!
  ...
  IF(NPAS /= 0)THEN  
    ...
    CALL CHEM(CP, MU, EN)
    CALL RAD(CP,  RMS)
    WRITE (7, 1008) ZNORM, MU/XOP, EN/XOP, RMS, (ABS(CP(1)/X(1)))**2
    1008 FORMAT('After NPAS iter.:',F8.4, 2F12.4, 2F11.4)
!
     OPEN(3, FILE = 'realsph-th-den.txt')
     CALL WRITE_DENSITY(3, CP)
    CLOSE(3)
!
  END IF
!
...
!
  ...
  IF(NRUN /= 0)THEN    
    ...
    CALL CHEM(CP, MU, EN)
    CALL RAD(CP,  RMS)
    WRITE (7, 1006) ZNORM, MU/XOP, EN/XOP, RMS, (ABS(CP(1)/X(1)))**2
    1006 FORMAT('After NRUN iter.:',F8.4, 2F12.4, 2F11.4)
!
!    OPEN(4, FILE = 'realsph-th-den-nrun.txt')
!      CALL WRITE_DENSITY(4, CP)
!    CLOSE(4)
!  
  END IF
...
!
  ...
  DO J = 0, N
    X(J) = J * DX
    X2(J) = X(J) * X(J)
    IF (NSTP == 0) THEN!READ OUTPUT of IMAGTIME PROGRAM WITH SAME PARAMETERS 
!      WRITE(*,'(a)') "Run the program using the input file to read. e.g.: ./real1d < imagcir-th-den.txt"
          READ (*,*)TX, TMP
          CP(J) = SQRT(TMP)
    ELSE
    CP(J) = X(J) * EXP(-X2(J) / 2.D0) / PI4
    END IF 
  END DO
  !$OMP END PARALLEL DO
END SUBROUTINE INITIALIZE
...
!
  CP(0) = 0.D0
  !$OMP PARALLEL DO PRIVATE(I, P2, TMP1D)
  DO I = 1, N
    P2 = ABS(CP(I)) * ABS(CP(I)) / X2(I)
    TMP1D = DT * (V(I) + G * P2) 
    CP(I) = CP(I) * EXP(-CI*TMP1D)
  END DO
  !$OMP END PARALLEL DO
END SUBROUTINE CALCNU
...
!----------------------------------------------------------------------
  ...
  END INTERFACE
!----------------------------------------------------------------------
  REAL (8), DIMENSION(1:SIZE(CP)-1) :: P2, DP2, GP2
  REAL (8), DIMENSION(0:SIZE(CP)-1) :: P, DCP, TMP1D, EMP1D
!
  P = ABS(CP)
  DCP = DIFF(P, DX)
  DP2 = DCP(1:)*DCP(1:)
...
!   
      ...
      EMP1D(0) = DP2(1)
      EMP1D(1:) = (V(1:) + GP2/X2(1:)/2.D0)*P2 + DP2    
! 
  MU = 4.D0*PI*SIMP(TMP1D, DX)
  EN = 4.D0*PI*SIMP(EMP1D, DX)
END SUBROUTINE CHEM
...
!
  DO I = 0, N
    IF (I == 0) THEN
        WRITE(FUNIT, 999) X(I), (ABS(U(1))/X(1))**2
    ELSE
        WRITE(FUNIT, 999) X(I), (ABS(U(I))/X(I))**2
    END IF
  END DO
  999 FORMAT(F12.4, E17.7E3)
END SUBROUTINE WRITE_DENSITY

!# File name : realsph-th.f90
